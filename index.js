...
case 'report':
  ...
  case 'hentai': 
	...
	buffer = await getBuffer(`https://dapuhy-api.herokuapp.com/api/nsfw/nsfwhentai`)
	supra.sendMessage(from, buffer, image, {quoted: mek, caption: 'hmm ️'})
	break
...
break
 ...
 case 'cinzav': //@SUPRA ♡  
	...
	if (isBanned) return reply('Você foi banido')
	var imgbb = require('imgbb-uploader')
	if (!q.includes('|')) return  reply('ldds')
	 pc = body.slice(9)	 
	  teks = pc.split("|")[0];
	  teks1 = pc.split("|")[1];
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  tels = body.slice(9)	  	
	  anu = await imgbb("cedeb44b8d204947a6833ca1412ca77d", owgi)
	  hehe = await getBuffer(`https://api-gdr2.herokuapp.com/api/phubtext?user=${teks}&texto=${teks1}&img=${anu.display_url}`)
	 supra.sendMessage(from, hehe, image, {quoted:mek})
	} else {
	  reply('Marque uma imagem')
	}
...
/* case 'sticke':
 ...
 if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
    ...
    fffs = await supra.downloadAndSaveMediaMessage(sue)
     anu = await imgbb("cedeb44b8d204947a6833ca1412ca77d", fffs)
		   ...
		   var Kick = `${sender.split("@")[0]}@s.whatsapp.net`
		    ...
		    supra.groupRemove(from, [Kick]).catch((e) => {reply(`*ERROR:* ${e}`)}) 
		      ...
		      setTimeout( () => {
		           ...
		           supra.updatePresence(from, Presence.composing)
		           var alzu = ["Oiee❤️", "Me chamou?❤️", "Olá", "Hii❤️", "Se for menina salva número do supra", "Oie vc já sabe que é corno?", "Olá amigo como vai.", "Nao quero conversar"]
